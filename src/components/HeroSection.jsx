...
// === TYPEWRITER HOOK ===
function useTypewriter(text, { startDelay = 0, speed = 80, isRevealed = true } = {}) {
  const [displayed, setDisplayed] = useState('');
  const [done, setDone] = useState(false);

  useEffect(() => {
    if (!isRevealed) { setDisplayed(''); setDone(false); return; }
  ...
  }, [text, startDelay, speed, isRevealed]);
  ...
  return (
    <div className="flex items-center justify-center gap-3 sm:gap-5 font-mono text-xs uppercase tracking-[0.15em] text-black/50">
      <div className="flex items-center gap-2">
        <span className="font-bold text-black/70">Based in Ghana</span>
      </div>
...
const TypewriterName = ({ isRevealed }) => {
  const line1 = 'CYRIL JERRY';
  const line2 = 'BAAH';

  const { displayed: text1, done: line1Done } = useTypewriter(line1, {
    startDelay: 600,
    speed: 75,
    isRevealed,
  });

  const { displayed: text2 } = useTypewriter(line2, {
    startDelay: line1Done ? 0 : 99999,
    speed: 75,
    isRevealed: isRevealed && line1Done,
  });

  const showCursor1 = !line1Done;
  const showCursor2 = line1Done;

  return (
    <>
      <h1 className="text-[clamp(4.25rem,14vw,9rem)] font-black uppercase tracking-tight text-black leading-[0.88] min-h-[1em]">
        {text1}
        {showCursor1 && (
          <span className="inline-block w-[0.08em] h-[0.85em] bg-black align-middle ml-1 animate-[blink_0.9s_step-end_infinite]" />
        )}
      </h1>
      <h1 className="text-[clamp(4.25rem,14vw,9rem)] font-black uppercase tracking-tight text-transparent leading-[0.88] mt-2 sm:mt-0 font-outline-fallback min-h-[1em]">
        {text2}
        {showCursor2 && text2.length < line2.length && (
          <span className="inline-block w-[0.08em] h-[0.85em] bg-black align-middle ml-1 animate-[blink_0.9s_step-end_infinite]" />
        )}
      </h1>
    </>
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center relative w-full mb-4 md:mb-5">
          {/* Left Decoration */}
          <OrbitingDecoration icon={Code2} delay={0.15} className="left-0 sm:left-2 lg:left-16 top-2" isRevealed={isRevealed} enableAmbientMotion={false} />
          <OrbitingDecoration icon={Terminal} delay={0.45} className="left-6 sm:left-12 lg:left-28 bottom-8 hidden sm:flex" isRevealed={isRevealed} enableAmbientMotion={false} />

          {/* Typewriter Name */}
          <TypewriterName isRevealed={isRevealed} />

          {/* Right Decoration */}
          <OrbitingDecoration icon={Database} delay={0.28} className="right-0 sm:right-2 lg:right-16 top-10" isRevealed={isRevealed} enableAmbientMotion={false} />
          <OrbitingDecoration icon={Cpu} delay={0.58} className="right-6 sm:right-12 lg:right-28 -bottom-2 hidden sm:flex" isRevealed={isRevealed} enableAmbientMotion={false} />
        </div>
