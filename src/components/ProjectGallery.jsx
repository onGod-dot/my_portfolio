...
// Returns the best src for a project image (local or cloudinary)
function projectImgSrc(img, width) {
  if (!img) return null;
  // Local path — return as-is (encode spaces)
  if (img.startsWith('/')) return encodeURI(img);
  // Cloudinary
  return cloudinarySrc(img, width);
}
...
export default function ProjectGallery({ onOpenProject }) {
  ...
  // Preload first image for smooth loading
  useEffect(() => {
    const src = projectImgSrc(PROJECT_META[0]?.img, 800);
    if (src) {
            ...
            >
              ...
              <div className="absolute inset-0 overflow-hidden">
                {project.img ? (
                  <img
                    draggable="false"
                    src={projectImgSrc(project.img, 800)}
                    alt={project.title}
                  ...
                  <div className="h-full w-full bg-neutral-800 flex items-center justify-center">
                    <span className="font-mono text-white/20 text-xs uppercase tracking-widest">Preview Available</span>
                  </div>
                )}
              </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 z-10">
                ...
                <div className="mt-3 flex items-center gap-3">
                  ...
                  {project.previewUrl && (
                    ...
                    >
                      <span className="font-mono text-[10px] uppercase tracking-[0.14em] font-bold">Live</span>
            ...
            >
              <div className="absolute inset-0 overflow-hidden bg-neutral-950">
                {project.img ? (
                  ...
                  <div className="h-full w-full bg-neutral-800 flex items-center justify-center">
                    <span className="font-mono text-white/20 text-sm uppercase tracking-widest">Preview Available</span>
                  </div>
                )}
              </div>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    {project.previewUrl && (
                      <a
                        ...
                        rel="noopener noreferrer"
                        onClick={(e) => e.stopPropagation()}
                        className="px-4 py-2 bg-white/10 border border-white/20 text-white font-mono text-xs uppercase tracking-wider flex items-center gap-2 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-lime-400 hover:text-black hover:border-lime-400"
                      >
                        Live Preview
                        <ArrowUpRight size={14} strokeWidth={2} />
                      </a>
                    )}
                    <div className="w-14 h-14 bg-white/10 border border-white/20 text-white flex items-center justify-center rounded-full group-hover:bg-lime-400 group-hover:text-black group-hover:border-lime-400 transition-all duration-300 shadow-lg">
